  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-bg">
      <div className="w-full max-w-sm">
        ...
        {/* Card */}
        <div className="bg-surface rounded-3xl p-6 border border-line shadow-2xl">
          {/* Icon + title */}
          <div className="text-center mb-6">
            <div className="w-14 h-14 bg-surface2 rounded-2xl flex items-center justify-center text-2xl mx-auto mb-4">
              {isLocked ? '🔒' : '✦'}
            </div>
          ...
          {!isLocked && (
            <div className="mb-4">
              <label className="block text-muted text-sm font-medium mb-1.5">
                Pseudonyme <span className="text-faint font-normal">(optionnel)</span>
              </label>
              <input
                type="text"
                value={pseudonym}
                ...
                className="w-full bg-surface2 border border-line focus:border-red-500 text-ink rounded-xl px-4 py-3 text-sm placeholder:text-faint outline-none transition-colors"
              />
            </div>
          )}
          ...
          {isLocked && !needsRecovery && (
            <button
              ...
              className="w-full text-faint hover:text-muted text-xs mt-3 transition-colors disabled:opacity-40"
            >
              Problème de connexion ? Recréer le passkey sur cet appareil
            </button>
          )}

          <p className="text-faint text-xs text-center mt-4 leading-relaxed">
            {needsRecovery
